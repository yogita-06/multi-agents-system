...
router.get('/stream', async (req, res) => {
  ...
  try {
    ...
    send('agent-complete', {
      agent: 1,
      status: 'complete',
      preview: researchFindings.substring(0, 250).trim() + '...',
    ...
    send('agent-complete', {
      agent: 2,
      status: 'complete',
      preview: analysis.substring(0, 250).trim() + '...',
      fullText: analysis,
    ...
    send('agent-start', {
      agent: 3,
      status: 'processing',
      message: '✍️ Agent 3 writing report...',
    });
    ...
    send('agent-complete', {
      agent: 3,
      status: 'complete',
      preview: (report.executiveSummary || '').substring(0, 250).trim() + '...',
      message: '✓ Report complete',
    });

    // Count words in the full report
    const fullText = [
